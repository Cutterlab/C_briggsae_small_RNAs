...
# This script uses gene annotations for genes and piRNAs
# (lifted_c_briggsae_HK104_NGM_Alignments_canonical_geneset_with_piRNAs.gtf), repeats 
...
# read in genes file
genes <- "lifted_c_briggsae_HK104_NGM_Alignments_canonical_geneset_with_piRNAs.gtf"
genes <- import(genes)
...
save(gene.whole, file="WS272.HK104_gene.whole.transposons.repeats_with_piRNAs.RData")
...
# features
gtf_file <- "lifted_c_briggsae_HK104_NGM_Alignments_canonical_geneset_with_piRNAs.gtf"
gtf <- import(gtf_file)
gtf <- gtf[seqnames(gtf) %in% base.chromosomes]
gtf <- c(gtf)


gtf.genes <- lapply(unique(grep("(WBGene)", gtf$gene_id, value=TRUE)), function(gene, gtf){

  print(grep(gene, unique(grep("(WBGene)", gtf$gene_id, value=TRUE))))
...
save(gtf.genes, file="WS272.HK104_gene.parts_with_piRNAs.RData")
...
save(gene.parts, file="WS272.HK104_gene.parts.transposons.repeats_with_piRNAs.RData")
