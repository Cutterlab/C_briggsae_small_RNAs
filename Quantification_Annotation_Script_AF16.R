...
# read in genes file
genes <- "c_briggsae.PRJNA10731.WS272.canonical_geneset.gtf"
genes <- import(genes)
...
save(gene.whole, file="WS272.gene.whole.transposons.repeats.RData")
...
# features
gtf_file <- "c_briggsae.PRJNA10731.WS272.canonical_geneset.gtf"
gtf <- import(gtf_file)
gtf <- gtf[seqnames(gtf) %in% base.chromosomes]
gtf <- c(gtf)


gtf.genes <- bplapply(unique(grep("(WBGene)", gtf$gene_id, value=TRUE)), function(gene, gtf, counter){
...
save(gtf.genes, file="WS272.gene.parts.RData")
...
save(gene.parts, file="WS272.gene.parts.transposons.repeats.RData")
